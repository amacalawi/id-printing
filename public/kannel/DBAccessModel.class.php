...
class DBAccess extends BaseModel
{
    ...
    function save_messages($detail){
        $school_year = $this->get_current_sy();
        $details = array(
            'body' => $detail['body'],
        ...
        return $this->insert('inbox', $details);
    }
    ...
    function get_current_sy()
    {
        $sql = "SELECT id FROM schoolyears where status = 'CURRENT'";
    ...
    function send($id,$msisdn,$smsc,$body,$memid=''){
        $d = Array();
        $d['schoolyear_id'] = $this->get_current_sy();
        $d['message_id'] = $id;
        if(!empty($memid)) {
            $d['member_id'] = $memid;
        }
    ...
    //    $url = SEND_URL . '&to=' . $msisdn . '&text=' . urlencode($body) . '&smsc=' . $smsc . '&dlr-url=' . urlencode($dlr) . '&coding=2' . '&charset=UTF-8';
    ...
    function group_members($group){
        ...
        $sql = "SELECT m.msisdn,m.id
                ...
                JOIN members as m ON m.id = gm.member_id WHERE g.groups_code='$group' AND gm.schoolyear_id = ".$school_year." AND m.removed_at IS NULL";
        return $this->select_all($sql);
         // AND gm.schoolyear_id = '$school_year' AND m.removed_at = NULL
    }
...
//        $sql = "SELECT typ.types_code FROM `members` mem JOIN types typ WHERE mem.`msisdn` = '$msisdn' AND mem.`msisdn` != '';";
	...
	//$sql ="SELECT typ.types_code FROM `members` mem JOIN types typ ON typ.types_id = mem.type WHERE mem.`msisdn` LIKE '%$msisdn%' AND mem.`msisdn` != ''";
    #$sql ="SELECT typ.types_code FROM `types` AS typ JOIN enrollments as enroll ON typ.types_id = enroll.type JOIN members as mem WHERE mem.`msisdn` LIKE '%$msisdn%' AND mem.`msisdn` != ''";
    $sql ="SELECT types.types_code FROM `members` JOIN `enrollments` ON enrollments.member_id = members.id LEFT JOIN types on types.types_id = enrollments.type LEFT JOIN schoolyears on schoolyears.id  = enrollments.schoolyear_id WHERE members.msisdn LIKE '%$msisdn%' AND schoolyears.status ='CURRENT' AND members.`msisdn` != ''";
    ...
    function check_if_existing($msisdn){
        $sql = "SELECT mem.id FROM `members` mem where mem.msisdn LIKE '%$msisdn%'";
        $res = $this->return_one($sql);
        return $res;
    }
    function check_if_its_existing($msisdn){
        $sql = "SELECT mem.id FROM `members` mem where mem.msisdn LIKE '%".$msisdn."%'";
        $res = $this->return_all($sql);
        return $res;
    }
    ...
    function get_sender_id($msisdn){
        $sql = "SELECT m.id FROM `members` m where m.msisdn LIKE '%$msisdn%'";
        $res = $this->return_one($sql);
        return $res;
    }
