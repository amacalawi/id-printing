...
class AppController
{
    ...
    function group_message($details){
        ...
        if (preg_match('/(.+?),(.+)/i',$details['message'], $m)) {
            ...
            if(strpos(trim($m[1]), $searchString) !== false) {
              foreach (explode('/',trim($m[1])) as $group) {
                $members = $this->db->group_members($group);
                if (count($members) < 1) {
                    $msg = str_replace('::group::', $group, $this->db->get_tmpl('group_does_not_exist'));
		...
		foreach ($members as $member){
			...
			foreach ($msisdns as $msisdn){
				$msisdn = trim($msisdn);
                    		$this->db->send_group($msisdn, $mid, $details['smsc'], $msg,$member['id']);
...
#                  $this->db->send_group($member['msisdn'], $mid, $details['smsc'], $msg,$member['id']);
    ...
    function reg($details){
        ...
        if (preg_match($pattern, $details['message'], $m)) {
               ...
               $mem['gender']=strtoupper(trim($m[5]));
               $mem['level']= 0;
...
//               $mem['type']='panunuluyan2010';
               ...
               $mem['created_by']= 1;
              $schoolyear = $this->db->get_current_sy();
 
               $gid=$this->db->get_group_id(trim($m[6]));
               ...
               $eid=$this->db->enroll('enrollments', $mid, $schoolyear);
               ...
               #INVALID GROUP CODE
               $msg = str_replace('::group_name::',trim($m[6]), $this->db->get_tmpl('invalid_group'));
