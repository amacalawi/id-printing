!function($) {
    ...
    wizzardS.prototype.validate_progressbar = function($init) {
        if ($init == 1) {
            var $requires = this.$body.find(".form-group.required").length - 2;
        } else {
        ...
        console.log('count: '+ $count + ', requires:' + $requires);
    },
    ...
    wizzardS.prototype.do_uploads = function($id) {
        ...
        $.ajax({
            type: "POST",
            url: base_url + 'applications/uploads?files=patents&id=' + $id,
            data: data,
            cache: false,
            processData: false,
            contentType: false,
    ...
    {   
        ...
        */
        this.$body.on('blur', 'input[name="sub_mailing_date2"]', function (e){
            ...
            if (self.val() != '') {
                ...
                $('input[name="professional_fee2_2"]').prop('disabled', false).closest('.form-group').removeClass('required');
                $.wizzardS.required_fields();
            }
        });
        ...
        */
        this.$body.on('click', 'input[name="subsequent_checklist_form_4"]', function (e) {
            ...
            if (self.val() == 'Yes') {
                ...
                $('.subsequent-form-4-1').find('input:not([type=radio])').prop('disabled', true);
                $('.subsequent-form-4-2').find('input').prop('disabled', false);
                $('input[name="subs_paper_no1"], input[name="subs_mailing_date1"]').closest('.form-group').removeClass('required').find('.m-form__help').text('');
                $('input[name="subs_paper_no2"], input[name="subs_mailing_date2"]').closest('.form-group').addClass('required').find('.m-form__help').text('');
                $.wizzardS.required_fields();
            } else {
                $('.subsequent-form-4-2').addClass('hidden');
                $('.subsequent-form-4-1').find('input:not([type=radio])').prop('disabled', false);
                $('.subsequent-form-4-2').find('input').prop('disabled', true);
                $('input[name="subs_paper_no1"], input[name="subs_mailing_date1"]').closest('.form-group').addClass('required').find('.m-form__help').text('');
                $('input[name="subs_paper_no2"], input[name="subs_mailing_date2"]').closest('.form-group').removeClass('required').find('.m-form__help').text('');
                $.wizzardS.required_fields();
            }
        });
            ...
            if ($('input[name="app_no"]').val() == '') { 
                ...
                */   
                ...
                {   
                    ...
                    */
                    var $url = (appID.val() != '') ? 
                        base_url + 'applications/patents/update/' + appID.val() + '?file1=' + inputfile1 + '&file2=' + inputfile2 + '&file3=' + inputfile3 + '&file4=' + inputfile4 + '&file5=' + inputfile5 + '&file6=' + inputfile6 + '&file7=' + inputfile7 : 
                        base_url + 'applications/patents/store?file1=' + inputfile1 + '&file2=' + inputfile2 + '&file3=' + inputfile3 + '&file4=' + inputfile4 + '&file5=' + inputfile5 + '&file6=' + inputfile6 + '&file7=' + inputfile7;
        ...
        */
        ...
        $('#app_filing_date, #pub_date').datepicker().on('changeDate', function (ev) {
            var self = $(this);
            var className = $(this).attr('name');
            var infoWidget = $('.app-widget-info');
            ...
            if (self.val() != '') {
                infoWidget.find('.' + className).text(self.val());
                if (self.attr('name') == 'app_filing_date') {
                    var m = moment(self.val(), 'MM/DD/YYYY');
                    var expiration = m.add(20, 'year');
                    infoWidget.find('.expiration_date').text(expiration.format('MM/DD/YYYY'));
                }

                if (self.attr('name') == 'pub_date') {
                    $.each($('body #annuity_table').find("tbody tr"), function(){   
                        var $row = $(this);
                        var $id = $row.attr('data-row-id');
                        var $m = moment(self.val(), 'MM/DD/YYYY');
                        var $annuity = $m.add($id, 'year');
                        console.log($annuity.format('MM/DD/YYYY'));
                        $row.find('input#annuity_due_date_'+ $id).attr('value', $annuity.format('YYYY-MM-DD'));
                        $row.find('input#annuity_due_date_'+ $id).val($annuity.format('YYYY-MM-DD'));
                        $row.find('input#annuity_due_date_clone_'+ $id).val($annuity.format('MM/DD/YYYY'));
                    });
                }
        ...
        this.$body.on('click', '.download-file', function (e){
            ...
            var $url = base_url + 'applications/downloads?files=patents&id=' + $id + '&filename=' + $filename;
